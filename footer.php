...
?>
    ...
    <footer id="colophon"
            class="site-footer <?php echo esc_html( get_theme_mod( 'bricks_footer_size', 'footer-full-width' ) ); ?>">
        <div class="footer_inner">
            <div class="container">
				<?php $bricks_footer_enable = get_theme_mod( 'bricks_layout_status', '0' );
				if ( ! empty( $bricks_footer_enable ) ) { ?>
                    <div class="footer-widget footer-content">
                        <div class="row">
							<?php $numberof_widgets = get_theme_mod( 'bricks_footer_layout', '4' );
							for ( $i = 1; $i <= $numberof_widgets; $i ++ ) {
								...
								<?php } elseif ( '2' === $numberof_widgets ) { ?>
                                    <div class="col-md-6 col-sm-6">
										<?php dynamic_sidebar( $foorert_widget ); ?>
                                    </div>
								<?php } elseif ( '3' === $numberof_widgets ) { ?>
                                    <div class="col-md-4 col-sm-6">
										<?php dynamic_sidebar( $foorert_widget ); ?>
                                    </div>
								<?php } elseif ( '4' === $numberof_widgets ) { ?>
				...
				<?php } ?>
				<?php if ( get_theme_mod( 'bricks_copy_write_text_status', '1' ) &&  get_theme_mod( 'bricks_social_icon_status','0' ) ) {
					$menu_icon = 'text_with_social';
				} else {
					$menu_icon = '';
				} ?>
                <div class="bricks_col_main footer-bottom <?php echo esc_html( get_theme_mod( 'bricks_social_layout', 'right-menu' ) ); ?> <?php echo esc_html( $menu_icon ); ?>">
					<?php if ( get_theme_mod( 'bricks_copy_write_text_status', '1' ) ) { ?>
                        <div class="bricks_col copyright_block">
                            <div class="site-info">
								<?php echo wp_kses_post( balanceTags( get_theme_mod( 'text_setting', 'Proudly powered by WordPress.' ) ) ); ?>
                            </div><!-- .site-info -->
                        </div>
					<?php } ?>
					<?php if ( get_theme_mod( 'bricks_social_icon_status', '0' ) ) { ?>
